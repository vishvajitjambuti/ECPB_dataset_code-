...
tf.app.flags.DEFINE_string('out_dir', 'data/tfrecords', 'Place to search for the created files.')
tf.app.flags.DEFINE_string('dataset_name', 'ecp-day',
                           'Name of the dataset, used to create the tfrecord files.')
tf.app.flags.DEFINE_string('anno_path', 'data/ecp/labels',
                           'Base directory which contains the ecp annotations.')
tf.app.flags.DEFINE_string('img_path', 'data/ecp/img',
...
def process_dataset():  # sourcery skip: use-fstring-for-concatenation, use-fstring-for-formatting
    ...
    np.save(os.path.join(FLAGS.out_dir, FLAGS.dataset_name + '-img_size_height_width'), list(img_sizes))


def create_dirs(dirs):  # sourcery skip: use-fstring-for-formatting
    for dir in dirs:
        try:
            os.makedirs(dir)
        except OSError:
